...
describe('app controller', () => {
  ...
  beforeAll(async () => {
    ...
    server = (await koaServer()).callback();
  });
  ...
  it('should create an app', async () => {
    const { body } = await request(server).post('/api/apps').send({ name: 'Test App', defaultPage: 'Test Page' });
  ...
  it('should update an app', async () => {
    ...
    expect(response.body).toEqual({ id: appA.id, name: 'Foobar', defaultPage: appA.definition.defaultPage });
  });

  it('should validate an app on creation', async () => {
    const response = await request(server).post('/api/apps').send({ foo: 'bar' });
