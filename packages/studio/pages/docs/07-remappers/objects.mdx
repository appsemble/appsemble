...
In the [Remapper tutorial](../07-remappers/), we referred to the Spotify song data that contains a
ton of properties with different types of data like `acoustincness`, `danceability`, and more.
There, we filtered the data by only referencing certain properties in the table block. This still
means the data is there, however. If you want to store this data somewhere, it still contains the
...
<RemapperRef remappers={objectRemappers} />
