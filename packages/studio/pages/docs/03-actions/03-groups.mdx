...
group members. To know more about the groups visit [this page](../02-guides/groups.md). Group
actions are client side actions, i.e. these actions are not implemented for the server side.
Supported group actions are as follow.
...
action requires the `$group:query` app permission. For more information check out [security](../02-guides/security.md#permissions). Here
is an example of how you can use this action.
...
the user will be invited, `email` is the email of the user to be invited to join the group and `role`
is the role as which the user is being invited. The `role` must exist in the roles property of the
app's security definition or in the predefined app roles in the system - `Member`, `MembersManager`,
`GroupMembersManager`, `GroupsManager`, `ResourcesManager` and `Owner`. This action requires the
`$group:member:invite` app permission. For more information check out [security](../02-guides/security.md#permissions).Here is an example
of how you can use this action.
...
[security](../02-guides/security.md#permissions).Here is an example of how you can use this action.
...
requires the `$group:member:delete` app permission. For more information check out [security](../02-guides/security.md#permissions). Here
is an example of how you can use this action.
