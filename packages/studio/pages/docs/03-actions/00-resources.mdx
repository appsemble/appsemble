...
and view is the view to use for the resource. This action requires at least one of the app
...
out [security](../02-guides/security.md#permissions). Here’s an example of how you can use this
action to fetch all `person` resources.
...
refers to the resource whose count is to be fetched. This action requires at least one of the app
permissions `$resource:all:query` or `$resource:<resource-name>:query` depending on the context. For
more information check out [security](../02-guides/security.md#permissions). Here’s an example of how you can use this action to fetch the
number of resources.
