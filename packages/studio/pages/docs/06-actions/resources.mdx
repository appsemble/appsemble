...
- [`resource.getHistory`](#resourcegethistory)
...
### resource.getHistory<Side name="resource.getHistory" />

This action is implemented for client side only. This action is used to fetch the history of a
single resource, `history` attribute of the resource should be set to true for this action to work.
Supported fields for this action are `type` and `resource`. Fields `type` and `resource` are
required. Resource refers to the resource to be fetched. This action requires at least one of the
app permissions `$resource:all:getHistory` or `$resource:<resource-name>:getHistory` depending on
the context. For more information check out [security](../02-guides/security.md#permissions). Here’s
an example of how you can use this action to fetch the `person` resource.
...
the context. For more information check out [security](../02-guides/security.md#permissions). Here’s
an example of how you can use this action to fetch the number of resources.
...
`$resource:<resource-name>:create` depending on the context. For more information check out
[security](../02-guides/security.md#permissions). Here’s an example of how you can use this action
to create a new resource of type person.
...
For more information check out [security](../02-guides/security.md#permissions). Here’s an example
...
context. For more information check out [security](../02-guides/security.md#permissions). To delete
a person resource, you can use the following block of code.
