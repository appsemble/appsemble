...
[request action](../08-reference/action.mdx#request). To authenticate request actions so data can be
...
> [proxy](../08-reference/action.mdx#request-proxy) property. If this is set to `false` the request
...
> [cronjobs](../03-guides/cron.mdx#actions).
