...
Appsemble provides a way to run [actions](../07-reference/action.mdx) based on a schedule through
cron jobs.
...
[user](../06-remappers/04-data.mdx#user) remapper won’t work the same.
...
<ul>
  {Array.from(serverActions).map((action) => (
    <li key={action}>
      <a href={`/docs/07-reference/action#${action}`}>{action}</a>
    </li>
  ))}
</ul>
...
[cron](../07-reference/app.mdx#-app-definition-cron) property.
