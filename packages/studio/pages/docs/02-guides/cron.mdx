...
Appsemble provides a way to run [actions](../05-reference/action.mdx) based on a schedule through
cron jobs.
...
[user](../04-remappers/04-data.mdx#user) remapper won’t work the same.
...
<ul>
  {Array.from(serverActions).map((action) => (
    <li key={action}>
      <a href={`/docs/05-reference/action#${action}`}>{action}</a>
    </li>
  ))}
</ul>
...
[cron](../05-reference/app.mdx#-app-definition-cron) property.
