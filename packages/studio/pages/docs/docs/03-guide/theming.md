...
to inner elements of the form block also unique paths generated by Appsemble’s normalization
function.

Even though direct theming and styling for inner elements of the blocks is not available via the
Appsemble idea it still allows to use CSS combinators, separators and selectors.
