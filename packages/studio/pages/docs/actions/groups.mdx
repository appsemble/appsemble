...
<ActionRef action={referenceSchemas.groupActions['group.query']} />
...
### group.selected.update<Side name="group.selected.update" />

<ActionRef action={referenceSchemas.groupActions['group.selected.update']} />

This action changes the currently selected group for the logged in app member. The `groupId` must be
either `null` (to clear the selected group) or the id of a group from the current `group.query`
result. If an invalid value is provided, the action throws an error. Here is an example of how you
can use this action.
...
<ActionRef action={referenceSchemas.groupActions['group.member.invite']} />
...
### group.member.create<Side name="group.member.create" />

<ActionRef action={referenceSchemas.groupActions['group.member.create']} />

This action allows a user with sufficient permissions to add app members to a group. Unlike the
`group.member.invite` action, this action does not create an invite but creates a new group member
with the specified **appMemberId** and **id**. Similar to the `group.member.invite` action, this
action validates the specified role. This action requires the `$group:member:invite` app permission.
Here is an example of how you can use this action.
...
<ActionRef action={referenceSchemas.groupActions['group.member.query']} />
...
<ActionRef action={referenceSchemas.groupActions['group.member.delete']} />
...
<ActionRef action={referenceSchemas.groupActions['group.member.role.update']} />
