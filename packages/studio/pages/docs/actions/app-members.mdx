...
The `roles` property must only contain roles from the app's security definition or from the
...
[security](../app/security.md#permissions). Here is an example of this action in use to update
multiple roles for an app member.
