...
- [`resource.update.group`](#resourceupdategroup)
...
out [security](../app/security.md#permissions).

The returned resource includes a server-managed `$etag` field. `resource.update`, `resource.patch`,
and `resource.delete` automatically send it back as `If-Match` so concurrent writers see a
`412 Precondition Failed` instead of silently overwriting each other. View responses
(`view: <view-name>`) omit `$etag`; downstream writes against view results therefore run without
`If-Match` and are not protected.

Here’s an example of how you can use this action to fetch the `person` resource.
...
### resource.update.group<Side name="resource.update.group" />

<ActionRef action={referenceSchemas.resourceActions['resource.update.group']} />

This action is used to move a resource from one group to another. This is useful when you need to
reassign resources between groups within your app.

**Requirements**:

This action requires the following permissions:

- `$resource:<resource-name>:delete` or `$resource:<resource-name>:own:delete` permission in the
  source group (the group the resource is being moved from)
- `$resource:<resource-name>:create` permission in the target group (the group the resource is being
  moved to)

For more information check out [security](../app/security.md#permissions).

Here is an example of how you can use this action to move a resource to a different group:
  ...
  - type: action-button
    ...
    actions:
      onClick:
        ...
        onSuccess:
          type: message
          body: Resource moved to new group
```
...
Optimistic concurrency is automatic. Resource responses include a server-managed `$etag` field, the
same way they include `$author` and `$updated`. When the action's input data carries a `$etag`
(typically the object returned by a prior `resource.get`, or an item from a `resource.query`
result), Appsemble sends the value as an `If-Match` header. If another writer changed the resource
in the meantime, the request fails with `412 Precondition Failed`; fetch the latest resource and let
the user review before retrying. Input that does not carry `$etag` — for example data loaded via a
view, or hand-rolled objects — is sent without `If-Match` and is not protected against lost updates.

For idempotent changes that can be merged automatically, use `optimistic`. This fetches the latest
resource before writing, exposes it as `{ context: resource }`, sends its `$etag` as `If-Match`, and
retries if another write wins first.
      ...
      onSubmit:
        ...
        onError:
          ...
          then:
            ...
            onSuccess:
              type: message
              body: This person was updated elsewhere. Review the latest version and try again.
          else:
            type: throw
```
...
As with `resource.update`, optimistic concurrency is automatic: the `$etag` present in the action's
input data (typically from a prior `resource.get`) is sent as `If-Match`, and a conflicting
concurrent write returns `412 Precondition Failed`. Input without `$etag` — including view results —
runs unprotected.
          ...
          - array.map:
              if:
                condition:
                  equals:
                    - { array: index }
                    - [{ root: null }, { prop: taskIndex }]
                then:
                  object.from:
                    title: { prop: title }
                    completed: true
                    completedBy: { app.member: email }
                else:
                  object.from:
                    title: { prop: title }
                    completed: { prop: completed }
                    completedBy: { prop: completedBy }
```
...
context. For more information check out [security](../app/security.md#permissions).

As with `resource.update` and `resource.patch`, optimistic concurrency is automatic: if the action's
input data carries `$etag`, it is sent as `If-Match` and the request fails with
`412 Precondition Failed` when the resource was changed in the meantime.

To delete a person resource, you can use the following block of code.
