...
same way they include `$author` and `$updated`. When that resource (or any object containing its
`$etag`) is the input to `resource.update`, Appsemble sends the value as an `If-Match` header. If
another writer changed the resource in the meantime, the request fails with `412 Precondition
Failed`; fetch the latest resource and let the user review before retrying.
...
As with `resource.update`, optimistic concurrency is automatic: the `$etag` on the resource flowing
into the action is sent as `If-Match`, and a conflicting concurrent write returns
