...
`resource` and `view`. Fields `type` and `resource` are required. Resource refers to the resource to
be fetched, and view is the view to use for the resource. This action requires at least one of the
app permissions `$resource:all:get`, `$resource:all:own:get`, `$resource:all:get:<view-name>`,
...
field for this action are `type` and `resource`. Both of the fields are required. Resource refers to
the resource to be updated. This action requires at least one of the app permissions
