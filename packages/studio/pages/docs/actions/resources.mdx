...
import { resourceActions as actions } from '../../../../utils/reference-schemas/actions/resources.ts';
...
<ActionRef action={actions['resource.get']} />
...
<ActionRef action={actions['resource.history.get']} />
...
<ActionRef action={actions['resource.query']} />
...
<ActionRef action={actions['resource.count']} />
...
<ActionRef action={actions['resource.create']} />
...
<ActionRef action={actions['resource.update']} />

This action is used to replace some resource’s data with a completely new set of values. Id can be
specified explicitly using the parameter `id` or appsemble can infer it from the data passed to the
action function, explicitly defined id is preferred by the system. This action requires at least one
of the app permissions `$resource:all:update`, `$resource:all:own:update`,
...
<ActionRef action={actions['resource.patch']} />

This action is used to partially update a resource with new data. This action requires at least one
of the app permissions `$resource:all:patch`, `$resource:all:own:patch`,
`$resource:<resource-name>:patch` or `$resource:<resource-name>:own:patch`, depending on the
context. For more information check out [security](../app/security.md#permissions). Here’s an
example of how you can partially update a resource of type person using this action.
...
<ActionRef action={actions['resource.delete']} />
...
<ActionRef action={actions['resource.delete.all']} />
...
<ActionRef action={actions['resource.delete.bulk']} />
...
<ActionRef action={actions['resource.subscription.subscribe']} />
...
<ActionRef action={actions['resource.subscription.unsubscribe']} />
...
<ActionRef action={actions['resource.subscription.toggle']} />

This action is used to toggle push notifications for resource modification. i.e. It will set the
subscribed property to `true` if it’s set to `false` and vice versa. Here’s
...
<ActionRef action={actions['resource.subscription.status']} />

This action can be used to access the status of notification subscriptions for a resource. Here’s an example of usage of this action.
