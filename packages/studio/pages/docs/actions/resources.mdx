...
To avoid overwriting a newer version, keep the `$etag` from the latest resource response and pass it
using `ifMatch`. If the resource changed in the meantime, the request fails with
`412 Precondition Failed`. In that case, fetch the latest resource again and ask the user to review
and retry.
