...
> For added security and to restrict the usage based on roles, use the [webhook](#webhook) action.
...
### webhook<Side name="webhook" />

<ActionRef action={referenceSchemas.miscellaneousActions['webhook']} />

The `webhook` action invokes a webhook defined in the app. Webhooks execute server-side, making them
ideal for secure operations like generating verification codes, sending emails, or modifying
resources without exposing sensitive logic to the client.

Users must have the `$webhook:<webhookName>:invoke` permission to call this action. The result of
the webhook's action chain is returned to the client.
