...
to the [contributing guidelines](/docs/contributing)
...
Throughout Appsemble we use the `<FormattedMessage />` component in place of static text. This
allows the studio to translate the text based on the user's locale.
...
Messages are defined in a file with the following format:
...
export const messages = defineMessages({
  title: {
    // Unique ID generated by formatjs based on the message
    id: 'studio.z+n8CE',
    // English-based default message to be translated
    defaultMessage: 'SomeTitle',
  },
});
```

For information on how to add messages yourself, please refer to the
[CONTRIBUTING.md](/docs/contributing#message-validation)
...
[react-components](https://gitlab.com/appsemble/appsemble/-/tree/main/packages/react-components) is
an internal package for Appsemble which contains React components. These components are used
everywhere in the Studio.

It's good practice to try and make your components as isolated as possible so they can be reused on
other parts of the project, and so that it's easy to test their individual functionality. In
Appsemble we place these reusable components under the `react-components` package.
