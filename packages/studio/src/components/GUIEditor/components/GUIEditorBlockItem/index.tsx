...
export default function GUIEditorBlockItem({ value }: GUIEditorBlockItemProps): React.ReactElement {
  const { name, onChange, value: currentValue } = useValuePickerProvider();

  const handleChange = React.useCallback((event) => onChange(event, value), [onChange, value]);
