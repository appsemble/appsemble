...
describe('getTeams', () => {
  ...
  it('should include the role of the user', async () => {
    ...
    await TeamMember.bulkCreate([
      { role: 'Member', AppMemberId: appMember.id, TeamId: teamA.id },
      { role: 'Manager', AppMemberId: appMember.id, TeamId: teamB.id },
    ]);
    ...
    expect(response).toMatchObject({
      status: 200,
      data: [
        { id: teamA.id, name: teamA.name, role: 'Member' },
        { id: teamB.id, name: teamB.name, role: 'Manager' },
