...
describe('getAppMember', () => {
  it('should return 404 if no app was found', async () => {
    ...
    const response = await request.get(
      '/api/common/apps/123/members/67ab4ea6-ce98-4f08-b599-d8fc4b460d37',
    );
  ...
  it('should return 404 if the app doesn’t have a security definition', async () => {
    ...
    authorizeStudio();
    const response = await request.get(
      `/api/common/apps/${app.id}/members/67ab4ea6-ce98-4f08-b599-d8fc4b460d37`,
    );
    expect(response).toMatchInlineSnapshot(`
      HTTP/1.1 404 Not Found
      Content-Type: application/json; charset=utf-8
  ...
  it('should return an app member if it is found', async () => {
    ...
    authorizeStudio();
    const response = await request.get<AppMemberType>(
      `/api/common/apps/${app.id}/members/${user.id}`,
    );
    expect(response).toMatchInlineSnapshot(
      {
        data: {
