...
describe('deleteGroupMember', () => {
  ...
  it('should remove a group member from a group', async () => {
    ...
    const response = await request.delete(`/api/apps/${app.id}/group-members/${groupMember.id}`);
  ...
  it('should remove a group member from a group if the user has the manager role', async () => {
    ...
    const response = await request.delete(
      `/api/apps/${app.id}/group-members/${groupMember.id}?selectedGroupId=${group.id}`,
    );
    expect(response.status).toBe(204);
  });
  ...
  it('should not remove a member who isn’t part of the group', async () => {
    ...
    const response = await request.delete(`/api/apps/${app.id}/group-members/${userB.id}`);
