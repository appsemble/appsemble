...
describe('updateAppScreenshot', () => {
  it('should replace an existing screenshot', async () => {
    const app = await App.create({
      definition: {},
      OrganizationId: organization.id,
      vapidPrivateKey: '',
    ...
    const form = createFormData({ screenshot: createFixtureStream('nodejs-logo.png') });
    const fixture = await readFixture('nodejs-logo.png');
    const response = await request.put(`/api/apps/${app.id}/screenshots/${screenshot.id}`, form, {
      headers: { authorization },
    });
    ...
    expect(screenshot.screenshot).toStrictEqual(fixture);
  });

  it('should not replace a non-existing screenshot', async () => {
    const app = await App.create({
      definition: {},
      OrganizationId: organization.id,
      vapidPrivateKey: '',
    ...
    const form = createFormData({ screenshot: createFixtureStream('nodejs-logo.png') });
    const response = await request.put(`/api/apps/${app.id}/screenshots/0`, form, {
      headers: { authorization },
    });
  ...
  it('should not replace an existing screenshot with a non-image', async () => {
    const app = await App.create({
      definition: {},
      OrganizationId: organization.id,
      vapidPrivateKey: '',
    ...
    const form = createFormData({ screenshot: Buffer.from('I am not an image.') });
    const response = await request.put(`/api/apps/${app.id}/screenshots/${screenshot.id}`, form, {
      headers: { authorization },
    });
