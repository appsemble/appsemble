...
setting basic colours and custom styling for advanced or more specific styling.

## Theme options

The Bulma CSS framework uses a set of colours for different parts of its elements. Appsemble
supports customizing a subset of these variables by including a `theme object` at different points
within an app recipe.
...
The `themeColor` and `splashColor` variables are not visible at all times. These define the colours
that appear when starting up the app after installing it on a mobile device.

Each variable can be assigned with a [`hex value`](hex) representing the desired colour.
...
The above example sets the app´s primary colour to green, the colour of its links to orange and the
colour of ´success´ elements to purple. The page `Red Page` overwrites the primary colour to be red
and one of its blocks overwrites the primary colour to be blue instead.
