...
export function FilterBlock({
  ...
  // Monotonically increasing id of the most recently started load, shared between filter
  // submissions and refresh events. Rapid interactions can make loads resolve out of order;
  // only the latest one may emit, or a stale response would overwrite newer data.
  ...
  const fetchData = useCallback(
    async (submitValues: FilterValues) => {
      ...
      const currentLoad = latestLoad.current;
      setLoading(true);
      try {
  ...
  return (
    <Form
      ...
      onSubmit={onSubmit}
    >
      <div className={classNames(styles['highlighted-fields-wrapper'], hasHighlight ? 'mb-6' : '')}>
